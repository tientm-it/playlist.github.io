...
export const getSetting = playplistId =>
  axios
    .get(`${API_URL}/playlists/view?playlistId=${playplistId}`)
    .then(result => {
      return result.data
    })
    .catch(error => {
...
export const getVideos = (playplistId, videoSettings) =>
  axios
    .get(`${API_URL}/videos/list?playlistId=${playplistId}`)
    .then(result => {
      let tempVideos = []
      let tempVideoIds = []
...
export const deleteVideo = (videoId, playplistId, callback) => {
  axios
    .get(`${API_URL}/videos/remove?videoId=${videoId}&playlistId=${playplistId}`)
    .then(res => {
